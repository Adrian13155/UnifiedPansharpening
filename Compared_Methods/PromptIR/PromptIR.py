...
class PromptIR_multitasks(nn.Module):
    def __init__(self, 
        ...
        self.patch_embed5 = OverlapPatchEmbed(5, dim)   
        self.patch_embed9 = OverlapPatchEmbed(9, dim)        
    ...
    def forward(self, inp_ms, inp_pan):
        ...
        if C1 == 4:
            inp_enc_level1 = self.patch_embed5(inp_img)
        else:
            inp_enc_level1 = self.patch_embed9(inp_img)
        ...
        if C1 == 4:
            out_dec_level1 = self.output4(out_dec_level1) + inp_ms
        else:
            out_dec_level1 = self.output8(out_dec_level1) + inp_ms

        return out_dec_level1
if __name__ == "__main__":

    torch.cuda.set_device(0)
    lr = torch.randn(1,8,32,32).cuda()
    pan = torch.rand(1,1,128,128).cuda()
    
    model = PromptIR_multitasks(dim = 16, num_blocks=[4, 6, 6, 8], decoder=True, prompt_dim=[20,40,104]).cuda()
