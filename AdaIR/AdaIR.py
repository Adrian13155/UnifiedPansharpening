...
class AdaIR(nn.Module):
    ...
    def forward(self, inp_img,noise_emb = None):
        ...
        if C == 4:
                out_dec_level1 = out_dec_level1.float().view(B, C, 2, H , W ).mean(dim=2)

        return out_dec_level1
    
class AdaIRwithPan(nn.Module):
    def __init__(self, 
        inp_channels=4, 
        out_channels=4, 
        dim = 48,
        ...
        super(AdaIRwithPan, self).__init__()
    ...
    def forward(self, inp_img, pan):
        ...
        if self.decoder:
            out_dec_level3 = self.fre2(pan, out_dec_level3)
        ...
        if self.decoder:
            out_dec_level2 = self.fre3(pan, out_dec_level2)
...
if __name__ == '__main__':
    ...
    lr = torch.randn(1,4,128,128).cuda()
    pan = torch.rand(1,1,128,128).cuda()
    model = AdaIRwithPan(inp_channels=8, 
        out_channels=8, 
        dim = 24,
        num_blocks = [2,3,3,4], 
