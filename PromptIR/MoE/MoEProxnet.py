...
class SpatialChannelPromptMoE(nn.Module):
    def __init__(self, 
        ...
        self.encoder_prompt_dim0 = SpaPromptGenBlock(spatial_prompt_num=5,spatial_prompt_size=128,dim=dim * 2 ** 0)
        self.encoder_prompt_dim1 = SpaPromptGenBlock(spatial_prompt_num=5,spatial_prompt_size=64, dim=dim * 2 ** 1)
        self.encoder_prompt_dim2 = SpaPromptGenBlock(spatial_prompt_num=5,spatial_prompt_size=32, dim=dim * 2 ** 2)
...
if __name__== "__main__":
    ...
    model = SpatialChannelPromptMoE(dim=16, num_blocks=[1,3,3,4], num_refinement_blocks = 2,heads = [1,2,4,8], ffn_expansion_factor = 2.66, bias = False, LayerNorm_type = 'WithBias', decoder = True)#.cuda()
    inp_ms = torch.rand(1, 4, 128, 128)#.cuda()
    

    output, loss= model(inp_ms)
