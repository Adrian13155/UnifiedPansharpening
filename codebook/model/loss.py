...
class Similarity(nn.Module):
    ...
    def forward(self, x, y):
        return torch.mean(torch.sum(x * y) / (torch.sqrt(torch.sum(x * x)) * torch.sqrt(torch.sum(y * y))))
...
class GramLoss(nn.Module):
    ...
    def forward(self, x, y):
        return F.l1_loss(gram(x), gram(y))
...
class L_con2:
    ...
    def __call__(self, x, y):
        ...
        for i in range(0, k - 1, b):
            temp.append(gram(y[i : i + b, :, :, :]))
        # y_hat_gram = [gram(fmap) for fmap in y_hat_features]
        L_style = 0
        for j in range(len(temp)):
            L_style += self.mse_sum(style_gram, temp[j])
        return L_style