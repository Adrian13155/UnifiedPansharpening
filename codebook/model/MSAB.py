...
from .vq import BlockBasedResidualVectorQuantizer
...
class MS_MSA(nn.Module):
    ...
    def forward(self, x_in):
        ...
        q = q.transpose(-2, -1)
        k = k.transpose(-2, -1)
        v = v.transpose(-2, -1) # v.shape = b x heads x dim_head x (hw)
...
class MCAB(nn.Module):
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                MC_MCA(dim=dim, dim_head=dim_head, heads=heads),
                PreNorm(dim, FeedForward(dim=dim))
            ]))
